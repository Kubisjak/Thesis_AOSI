...
file = fopen('MC_AO_No_Control_Variate_output.tex', 'w');
fprintf(file, '\\begin{tabular}{|rrrrrr|}\\hline \n');
fprintf(file, '$\\sigma$ & $K$ & Ve\\v{c}e\\v{r}''s price & MC Price & Variance & STD Error \\\\ \\hline \\hline \n');
...
%--------------------------------------------------------------------------
% Output:
    fprintf(file, '    & %8.0f & %8.2f & %8.2f & %8.4f & %8.4f \\\\ ',Param_K(j),price_call,aCallPrice_mean, var(aCallPrice),aCallPrice_STDerror);
    fprintf(file, '\n');
...
fclose(file);
movefile('MC_AO_NO_Control_Variate_output.tex','/Users/Kubisjak/Google Drive/Vyzkumak/Vypracovanie/Final/Matlab');
